...
import org.example.app.utils.QuotaValidator;
...
public class ProductService {
    ...
    public String create(Product product) {
        ...
        if (!errors.isEmpty()) {
            try {
                throw new ProductDataException("Перевірте введення...", errors);
    ...
    public String update(Product product) {
        ...
        if (!errors.isEmpty()) {
            try {
                throw new ProductDataException("\u2753 Перевірте введення...",
                        errors);
            } catch (ProductDataException e) {
                return e.getErrors(errors);
            }
    ...
    private void validateData(Product product) {
        ...
        if (QuotaValidator.isQuotaValid(product.getQuota()))
